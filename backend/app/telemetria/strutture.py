"""Le tre pagine di shared memory di ACC, byte per byte.

Fonte: «ACC Shared Memory Documentation v1.8.12» di Kunos Simulazioni — l'ultima
pubblicata — incrociata con un'implementazione di riferimento mantenuta
(PyAccSharedMemory, rrennoir). I nomi dei campi sono lasciati **identici a quelli
del documento**: e' l'unico modo di poterli ricontrollare riga per riga. La
traduzione in nomi PitWall avviene nel dizionario dei canali, non qui.

Ordine delle ruote in ogni array a 4: FL, FR, RL, RR.

Nota sulle dimensioni: la pagina fisica e' 800 byte e quella grafica 1588 — due
conti che tornano esatti sommando i campi del documento, e che i test verificano.
Per la pagina statica il conto da' **820**: alcune implementazioni in giro mappano
784 byte e cosi' facendo troncano l'ultimo campo (`wetTyresName`). Noi mappiamo la
dimensione vera e, se il gioco ne espone una minore, il lettore ripiega
dichiarando quali campi non ha potuto leggere.
"""
...
# Versione della struttura che questo codice sa leggere. Se un aggiornamento di
# ACC aggiunge campi in fondo, il lettore continua a leggere quelli che conosce.
VERSIONE_STRUTTURA = "1.8.12"


# ─────────────────────────── enumerazioni del gioco ───────────────────────────
...
class SPageFilePhysics(ctypes.Structure):
    """Local\\acpmf_physics — cambia a ogni passo di fisica. 800 byte."""
...
class SPageFileGraphics(ctypes.Structure):
    """Local\\acpmf_graphics — cambia a ogni passo grafico. 1588 byte."""
...
class SPageFileStatic(ctypes.Structure):
    """Local\\acpmf_static — scritta all'avvio, non cambia piu'. 820 byte."""
...
# ─────────────────── che cosa il gioco riempie davvero ────────────────────────
# Campi che il documento ufficiale marca «Not used / Not shown in ACC»: li
# leggiamo comunque (la struttura dev'essere completa o saltano gli offset), ma
# non devono finire nei canali registrati ne', peggio, in un'analisi.

CAMPI_NON_USATI: dict[str, frozenset[str]] = {
    "physics": frozenset({
        "wheelLoad", "tyreWear", "tyreDirtyLevel", "camberRAD", "drs", "cgHeight",
        "numberOfTyresOut", "kersCharge", "kersInput", "rideHeight", "ballast",
        "airDensity", "performanceMeter", "engineBrake", "ersRecoveryLevel",
        "ersPowerLevel", "ersHeatCharging", "ersIsCharging", "kersCurrentKJ",
        "drsAvailable", "drsEnabled", "tyreTempI", "tyreTempM", "tyreTempO",
        "P2PActivation", "P2PStatus", "mz", "fx", "fy", "suspensionDamage",
    }),
    "graphics": frozenset({"replayTimeMultiplier", "surfaceGrip"}),
    "static": frozenset({
        "maxTorque", "maxPower", "suspensionMaxTravel", "tyreRadius",
        "maxTurboBoost", "deprecated_1", "deprecated_2", "hasDRS", "hasERS",
        "hasKERS", "kersMaxJ", "engineBrakeSettingsCount",
        "ersPowerControllerCount", "trackSplineLength", "trackConfiguration",
...
# ─────────────────────────── le tre pagine mappate ────────────────────────────

PAGINE: dict[str, tuple[str, type[ctypes.Structure]]] = {
    "physics": ("Local\\acpmf_physics", SPageFilePhysics),
    "graphics": ("Local\\acpmf_graphics", SPageFileGraphics),
    "static": ("Local\\acpmf_static", SPageFileStatic),
}

# Dimensioni attese, ricavate sommando i campi del documento ufficiale.
DIMENSIONI_ATTESE: dict[str, int] = {
    "physics": 800,
    "graphics": 1588,
    "static": 820,
}
